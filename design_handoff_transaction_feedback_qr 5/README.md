...
7. **Fields** — gap 16px, inputs `min-height: 46px`, `font-size: 16px` (prevents iOS zoom):
   ...
   - **`Mã giao dịch trên vé / Transaction ID`** — optional, scan-or-type. Full spec in **C1-a** below.
...
### C1-a. Transaction ID field (`transactionId`)

Ties a feedback record to the specific ticket/POS transaction, so a rating can be joined to film, showtime, screen, seat and cashier rather than only to the staff QR token. **Optional — never blocks submit.**

**Where the value comes from.** The printed Galaxy ticket carries `Trans No: 01313035/0002` mid-ticket, and **two** QR codes. The correct one is the **middle QR** — directly beneath the `Trans No` / `Trans Time` lines and above `Vui lòng quét mã QRCode này để vào rạp`; it is the entry QR and encodes the transaction. The QR at the **foot** of the ticket (under `Quét QR để chia sẻ cảm nhận…`) is the feedback-link QR and must **not** be used for this field. Two acquisition paths, both supported:

| Path | How the user arrives | Field behaviour |
|---|---|---|
| **Ticket QR** (preferred) | The ticket-foot feedback QR encodes `/f/:qrToken?tx=<transactionId>` | Field is **pre-resolved on load** — no typing at all |
| **Station QR** (POS standee, Phase 1 default) | `/f/:qrToken` with no `tx` | Field starts idle: **scan button first**, manual entry as the secondary ghost action (decided — `txnEntry: scan-first`) |

**Recommendation:** put `tx` into the ticket-foot feedback-QR payload and treat the in-page scanner and the manual input purely as fallbacks. Manual entry of `01313035/0002` off thermal paper is slow and error-prone; expect most users to skip it if it is the only option.

**Four states**, rendered in place inside the field:

1. **Idle** — secondary full-width button `Quét mã QR trên vé` (`min-height: 52px`, 15px, with a 19px QR-frame icon, gap 10px), then an `hoặc` divider (1px `#E5E7EB` rules either side, 13px muted), then a ghost full-width `Nhập tay mã giao dịch / Enter manually` (`min-height: 44px`, 14px).
2. **Scanning** — a `#F5F5F5` box, 1px `#E5E7EB`, radius 2px, padding 22px, centered: a 96×96 viewfinder (2px `#034EA2` corner brackets + a 2px accent scan line at 48%), `Đưa mã QR ở cuối vé vào khung` 15px, `Point at the QR code printed on your ticket` italic muted, ghost `Không quét được? Nhập tay`. Production: `BarcodeDetector` where available, else a JS decoder (jsQR / zxing-wasm) over `getUserMedia`. Requires HTTPS; if camera permission is denied or unavailable, **fall through to manual immediately** with an inline note rather than an error screen. The prototype simulates a hit after 1400ms.
**"Which QR?" helper — popup.** The ticket carries **two** QR codes and users confuse them, so the field label row ends with a 26px circular `?` button (1px `#9DBDE4`, fill `#EAF1FA`, text `#02458F`, hover `#CBDCF1`) pinned right of the label. It opens a **popup**, not inline copy — the explanation is reference material, not part of the filling flow, and inline expansion pushed the whole form down.

Popup content (identical in all treatments):

- Kicker `VÉ CỦA BẠN CÓ 2 MÃ QR` · headline `Quét mã **số 1**` (the numeral in `#02458F`).
- **Ticket schematic**, 146px wide, monospace 7px, 1px `#E5E7EB`: masthead `GALAXY CINEMA / VE XEM PHIM`, dashed rule, film + showtime + seat, dashed rule, the `Trans No` / `Trans Time` lines, then **QR ① in a 2px `#034EA2` outline on an `#EAF1FA` field with a solid-accent numeral badge**, caption `MÃ CẦN QUÉT`, the ticket's own `Vui long quet ma QRCode nay de vao rap` line, dashed rule, `Quet QR de chia se cam nhan`, then **QR ② at 45% opacity with a grey ② badge** and `khong dung ma nay`. The QR blocks are drawn (checker fill + three finder rings), deliberately not photographs: legible at 40–56px, printable, and no real customer ticket is exposed.
- **Numbered legend** mirroring the badges: ① *Mã ở giữa vé — ngay dưới dòng `Trans No`* + English line; ② *Mã ở cuối vé — mã dẫn tới trang đánh giá này, không dùng ở đây* (muted). Rule, then `Không có vé giấy? Bỏ qua mục này — đánh giá vẫn gửi được bình thường.`
- Primary full-width `Đã hiểu` closes; scrim click and ✕ also close; content click does not (`stopPropagation`). Production: close on `Esc`, trap focus, return focus to the `?` button.

**Treatment — decided: bottom sheet.** Build `sheet`; the other two exist in the prototype only as rejected comparisons.

| Value | Treatment | Status |
|---|---|---|
| `sheet` **← build this** | Bottom sheet over a `rgba(17,24,39,.45)` scrim: full width, `border-radius: 12px 12px 0 0`, `--shadow-lg`, padding `10px 20px 24px`, a 38×4px `#D1D5DB` grab handle centered above the kicker | Chosen — thumb-reachable and native on mobile, which is the only customer surface |
| `modal` | Centered dialog, 18px inset, radius 3px, ✕ in the header | Rejected |
| `popover` | 334px card anchored under the label with a rotated-square arrow | Rejected — crowds a 390px viewport |

Sheet behaviour in production: slide up ~180ms ease-out, scrim fades; dismiss on scrim tap, `Đã hiểu`, `Esc`, or a downward drag past ~40% of its height; body scroll locked while open; focus trapped and returned to the `?` button on close.

3. **Manual** —3. **Manual** — text input, `inputmode="numeric"`, `font-variant-numeric: tabular-nums`, `letter-spacing: .04em`, placeholder `01313035/0002`; helper line `Dòng **Trans No** in ở giữa vé, ngay trên mã QR.` 12px muted; ghost back-link `← Quét QR thay vì nhập tay`. Validation is **format-only and live** (not gated on submit): `^\d{6,10}/\d{3,5}$`, message `Định dạng mã giao dịch chưa đúng (ví dụ 01313035/0002)`. An empty value is valid.
4. **Resolved** — confirmation block: 1px `#9DBDE4`, background `#EAF1FA`, radius 2px, padding 14px 16px. Kicker `VÉ ĐÃ NHẬN DIỆN` 11px uppercase `#02458F`, ghost `Đổi vé` right-aligned (returns to idle and clears the value), film title 17px, meta line 13px muted (`cinema · date time · screen · seat`), then `Trans No **01313035/0002**` 13px tabular.

**Resolution & trust.** After a scan or a completed manual entry, call `GET /api/v1/public/transaction/:transactionId` (rate-limited, same throttle bucket as the public feedback POST) to render the resolved ticket summary. It must return only display-safe fields — film, cinema, showtime, screen, seat — never price, member id or phone. A transaction that does not resolve is **not** an error: keep the raw string, show `Không tìm thấy vé — vẫn gửi được đánh giá`, and persist it as unverified. Store `transactionId` plus `transactionSource` (`QR_TICKET` | `QR_SCAN` | `MANUAL` | `NONE`) and `transactionVerified` (bool) on the feedback record; admin A2/A3 should surface the ticket context and let a manager filter feedback that has one.

**Anti-abuse.** `transactionId` joins phone and QR token as a duplicate-detection signal — more than one feedback against the same transaction inside the dedupe window gets the existing `nghi vấn` flag rather than a hard block.

**Prototype tweak.** The DC exposes `txnEntry` (`scan-first` | `manual-first` | `prefilled`) so the three entry paths can be compared without rebuilding; it is a demo control, not a production prop.
...
### A1. Dashboard (Cinema Manager / Head Office) — chart-led, revised

Rebuilt to be read as a chart page, not a table page: every block is a graphic, the two long tables moved off (feedback → A2, staff → A9/ranking) and are reached from the footer buttons. Kicker + h2 still change per role (`Cinema Manager · Galaxy Nguyễn Du` / `Dashboard rạp Nguyễn Du` vs `Head Office · toàn hệ thống` / `Dashboard toàn hệ thống`). No cards, no panel borders — sections are separated by 52–56px of whitespace only.

**Filter bar** — unchanged: segmented date range (Hôm nay / Hôm qua / 7 ngày / 30 ngày / Tuỳ chọn), cinema `<select>` (Head Office only), `Bao gồm feedback nghi vấn` checkbox (**default off**), `Xuất CSV`.

**1 · KPI strip with sparklines** — `repeat(auto-fit, minmax(170px, 1fr))`, gap 34px, 4 metrics (was 6; Neutral and Eligible-staff dropped as non-actionable):

| KPI | Value | Delta | Sparkline |
|---|---|---|---|
| Tổng feedback | 953 / 3.868 | `+8,4%` | 14-day volume, accent |
| Điểm trung bình | 4,62 / 4,51 | `+0,06` | 14-day rating, accent |
| Tích cực 4–5★ | 85% | `+2 điểm` | 7-point, accent |
| Tiêu cực 1–2★ | 7% | `−1 điểm` | 7-point, **accent-2** |

Each: 10px uppercase label · 40px serif number with the delta on the same baseline (12px 600, accent-700 when improving, accent-2-700 when worsening) · a 120×24 sparkline `<path>` (1.4px, no fill, no axis) · 11px italic English. Deltas are always **vs. the previous equal-length period**.

**2 · Lượng feedback & điểm trung bình** — the hero chart, one full-width dual-axis SVG (`viewBox 0 0 720 232`, plot box L42 R674 T18 B196):
- volume as a 2.5px accent line over a 12%-accent area fill, one 2.5px hollow dot per day, last point a filled 4.5px dot with its value called out in accent-700;
- average rating as a 1.75px accent-2 line, `stroke-dasharray 5 4`, on its own right-hand 4,0–5,0 scale, last value called out below the point;
- 5 horizontal gridlines (`--color-neutral-200`) with left-axis labels 0/25/50/75/100 (0–400 for Head Office), right-axis labels 4,0 / 4,5 / 5,0 in accent-2-700, a full-strength baseline rule, and every third day labelled on x;
- legend above the plot (solid = volume/left axis, dashed = rating/right axis).

**3 · Cơ cấu điểm** — rating mix as a single 34px 100% stacked ribbon, segments flexed by count: 5★ accent · 4★ accent-400 · 3★ neutral-200 · 2★ accent-2-400 · 1★ accent-2, each labelled inline with its percentage when the segment is ≥6% wide. Below it a swatch legend (`5★ 612 · 64%`). Replaces the old five-row bar list.

**4 · Lý do nổi bật** — positive and negative reasons merged into **one** ranked bar chart on a shared scale (max 402), sorted by count, bar colored accent for positive and accent-2 for negative, right column `count · share of all feedback`. Two separate reason lists were reading as two unrelated charts.

**5 · Feedback theo giờ & ngày trong tuần** — heatmap, 7 weekday rows (T2…CN) × 7 two-hour buckets (10–12 … 22–24), 30px cells, 3px gaps, no borders. Cell fill = `color-mix(accent 12–90%, bg)` by intensity; the number prints in each cell, flipping to `--color-bg` above 55% intensity for contrast. Legend: Ít → Nhiều with the max value stated. This is the staffing view — the peak is T7 18–20h.

**6 · Điểm trung bình theo rạp** (Head Office only) — replaces the comparison table with a lollipop/dot plot: rows `160px | track | 112px`, each track a 4,0–5,0 scale with a hairline, an accent-300 leader line, an 11px accent dot at the cinema's average, and a **full-strength vertical benchmark rule at the system average 4,51**. Right column `avg · n fb · neg%` (neg in accent-2-700). Row click → drill-down. Axis ticks 4,0 / 4,5 / 5,0 under the tracks.

**7 · Cần chú ý** — the only list left, capped at 3 rows: staff code + name, the alert reason (`NEG_BURST · 3 feedback ≤2★ / 60 phút`), a 7-day accent-2 sparkline, the average, and the negative count. Row click → A9 performance. Footer: `Xem tất cả feedback` (secondary) + `Bảng xếp hạng →` (ghost).

Chart geometry is computed in the logic layer (`dashData()` → path strings, tick positions, tint steps) and rendered as plain SVG/DOM markup — **no chart library**. When rebuilding in a real stack, any library is fine as long as the tokens hold: accent for volume/positive, accent-2 for rating/negative, gridlines at `--color-neutral-200`, no chart borders or legends inside boxes.
...
Customer: `rating`, `reasons[]`, `name`, `phone`, `comment`, `consent`, `transactionId`, `transactionMode` (idle / scanning / manual / resolved), `transactionHelpOpen`, `transactionSource`, `submitAttempted`, `submitting`, `screen` (form / thanks / invalid / error / privacy / loading). Config (`ratingType`, rating levels, reason list) arrives from `GET /api/v1/public/feedback-config` and the token validation from `GET /api/v1/public/feedback/:qrToken` during SSR.
