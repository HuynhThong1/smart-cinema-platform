...
**Where the value comes from.** The printed Galaxy ticket carries `Trans No: 01313035/0002` mid-ticket, plus two QR codes: the entry QR above it, and a *feedback* QR at the foot under `Quét QR để chia sẻ cảm nhận của bạn về Galaxy Cinema`. So there are two acquisition paths and the design supports both:

| Path | How the user arrives | Field behaviour |
|---|---|---|
| **Ticket QR** (preferred) | The ticket-foot QR encodes `/f/:qrToken?tx=<transactionId>` | Field is **pre-resolved on load** — no typing at all |
| **Station QR** (POS standee, Phase 1 default) | `/f/:qrToken` with no `tx` | Field starts idle: scan button first, manual entry second |

**Recommendation:** put `tx` into the ticket-foot QR payload and treat the in-page scanner and the manual input purely as fallbacks. Manual entry of `01313035/0002` off thermal paper is slow and error-prone; expect most users to skip it if it is the only option.
